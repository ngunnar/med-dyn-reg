...
def main(dim_z=16, 
    ...
    train_generator = TensorflowDatasetLoader(root = config.ds_path,
                                              ...
                                              size = config.ds_size, output_first_frame=True)
    ...
    train_dataset = train_dataset.shuffle(buffer_size=len_train//2).batch(config.batch_size)
    test_dataset = test_generator.data
    test_dataset = test_dataset.batch(config.batch_size)
    
    
    # model training
    model = UFLOW_KVAE(config = config)
    ...
    for epoch in range(model.epoch, config.num_epochs+1):
        ...
        if loss_training < best_loss and model.epoch >= config.only_vae_epochs:
            model.save_weights(model_log_dir + '/ukvae_flow_cardiac_best')
            best_loss = loss_training
            checkpoint.write(file_prefix=checkpoint_prefix)  # overwrite best val model
        model.epoch += 1
    model.save_weights(model_log_dir + '/ukvae_flow_cardiac_{}'.format(config.dim_z))
