...
from .losses import log_p_kalman, grad_loss
...
class FLOW_VAE(VAE):
    ...
    def call(self, inputs):
        ...
        self.update_loss(p_y_x, y, q_x_y, x, mask)
        grad = grad_loss('l1', tf.reshape(phi_y_x.mean(), (y.shape[0]*y.shape[1], y.shape[2], y.shape[3], 2)))
...
class KVAE(VAE):
    ...
    def get_latents(self, inputs):
        ...
        #x_mu_filt, x_covs_filt = self.kf.kalman_filter.latents_to_observations(mu_filt, Sigma_filt)
